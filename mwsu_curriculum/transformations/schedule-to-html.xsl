...
<xsl:template name="subjectInfo">
  ...
  <td><xsl:value-of select="csmp:meeting/csmp:StartTime"/> - <xsl:value-of select="csmp:meeting/csmp:EndTime"/></td>
  <td>
    <xsl:for-each select="csmp:meeting/csmp:day">
      <xsl:value-of select="."/>
    </xsl:for-each> 
  </td>
  <td><xsl:value-of select="csmp:meeting/csmp:building/csmp:buildingRoom"/></td>
  <td><xsl:value-of select="csmp:meeting/csmp:building/csmp:max"/></td>
  <td><xsl:value-of select="csmp:meeting/csmp:instructor"/></td>
  </tr>
  <xsl:call-template name="subjectInfo">
  </xsl:call-template>
  </xsl:if>
